...
export default {
  ...
  argTypes: {
    ...
    fontSize: {
      control: "text",
      description:
        "Any CSS length, including clamp(). Auto-shrinks if the text would overflow its container.",
      table: { category: "Content" },
    },
    fontWeight: {
    ...
    fontFamily: {
      control: "text",
      description: "`inherit` picks up the container's computed family.",
      table: { category: "Content" },
    },
    ...
    density: {
      ...
      description:
        "Sampling step in px — lower means more particles and crisper glyphs.",
      table: { category: "Particles" },
    },
    particleSize: {
      control: { type: "range", min: 0.5, max: 8, step: 0.1 },
      description:
        "Base particle size. At or below 2.1 they draw as squares, above that as circles.",
      table: { category: "Particles" },
    },
    color: {
      control: "color",
      description: "Colour at the left edge of the text.",
      table: { category: "Particles" },
    },
    highlightColor: {
      control: "color",
      description:
        "Colour at the right edge — particles blend between the two across the width.",
      table: { category: "Particles" },
    },
    glow: {
      control: "boolean",
      description:
        "Canvas shadow blur tinted with highlightColor. The single most expensive option here.",
      table: { category: "Particles" },
    },
    ...
    trigger: {
      ...
      description:
        "`mount` gathers once. `hover` and `click` re-scatter and re-gather on that interaction.",
      table: { category: "Motion" },
    },
    scatter: {
      control: { type: "range", min: 0, max: 600, step: 10 },
      description: "How far particles start from their target, in px.",
      table: { category: "Motion" },
    },
    gatherDuration: {
      control: { type: "range", min: 200, max: 5000, step: 100 },
      description: "Flight time per particle, in ms.",
      table: { category: "Motion" },
    },
    stagger: {
      control: { type: "range", min: 0, max: 2000, step: 20 },
      description: "Spread of start delays across the field, in ms.",
      table: { category: "Motion" },
    },
    idleDrift: {
      control: { type: "range", min: 0, max: 6, step: 0.1 },
      description: "Amplitude of the resting shimmer once assembled.",
      table: { category: "Motion" },
    },

    pointerRepel: {
      control: { type: "range", min: 0, max: 200, step: 5 },
      description: "How hard the pointer pushes particles away.",
      table: { category: "Pointer" },
    },
    repelRadius: {
      control: { type: "range", min: 0, max: 500, step: 10 },
      description: "Radius of the pointer's influence, in px.",
      table: { category: "Pointer" },
    },
...
export const LandingHeadline = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "The landing page settings exactly: left aligned, density 3, a 1.7s gather with 460ms of stagger. Reload the story to replay the assembly.",
      },
    },
  },
  render: (args) => (
...
export const GatherOnHover = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          '`trigger: "hover"` re-scatters and re-gathers every time the pointer enters. Move away and back to replay it.',
      },
    },
  },
  render: (args) => (
    <Stage height={200} width={620}>
      <ParticleText {...args} />
    </Stage>
  ),
};
...
export const GatherOnClick = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Same idea, driven by click — steadier if the text sits somewhere the pointer crosses often.",
      },
    },
  },
  render: (args) => (
...
export const DensityLadder = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "The same word at four sampling steps. Below about 3 the area cap starts thinning the field anyway, so the gain flattens out while the cost does not.",
      },
    },
  },
  render: (args) => (
...
export const NoGlow = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Glow costs a canvas shadow per particle. Turning it off is the first thing to try if the effect stutters on a weak GPU.",
      },
    },
  },
  render: (args) => (
...
export const Assembled = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Scatter and drift at zero, so the text sits still. This is roughly what a reduced-motion visitor sees, and it is the story to use for a stable screenshot.",
      },
    },
  },
  render: (args) => (
